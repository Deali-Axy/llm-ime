...
> ⚠️ **实验性项目**：目前处于 Web 验证阶段，核心引擎和交互逻辑仍在打磨中。下一步计划基于 RIME 框架或从头开发一个原生输入法前端，具体取决于后续规划。
...
目前以 **Web Dashboard** 作为验证载体：在浏览器里打字，实时看到候选词、输入统计和引擎状态，用于验证引擎逻辑的正确性和响应速度。Web 前端并非最终形态，待核心引擎稳定后，会进一步接入真实的输入法框架。
