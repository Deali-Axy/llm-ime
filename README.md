...
服务默认监听 `http://127.0.0.1:5000`（可通过 `PORT` / `LIME_PORT` 和 `HOST` / `LIME_HOST` 修改）。
...
| `HOST` / `LIME_HOST` | `127.0.0.1` | 服务监听地址，RIME 首版建议保持本机回环地址 |
| `LIME_SHARED_SECRET` | 空 | `/api/ime/*` 的可选 Bearer 密钥 |
| `LIME_SESSION_TTL_MS` | `900000` | IME 会话闲置过期时间（毫秒） |
